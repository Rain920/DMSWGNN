...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len:Optional[int]=1024, d_k:Optional[int]=None, d_v:Optional[int]=None, norm:str='BatchNorm', attn_dropout:float=0., 
        ...
        self.classifier = nn.Linear(self.seq_len * c_in, configs.num_classes)
        self.loss = nn.CrossEntropyLoss()
    
    
    def forward(self, x, label):           # x: [Batch, Input length, Channel]
        ...
        x = self.classifier(x)
        mse_loss = self.loss(x, label)
        loss = mse_loss + PaEN_Loss
        return x, loss
...
class HDMixer_backbone(nn.Module):
    def __init__(self,configs, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        self.wtconv_patch = configs.wtconv_patch
        if self.wtconv_patch:
            self.wt_levels = configs.wt_levels
            self.wt_name = configs.wt_name
            if self.wt_levels > 1:
                self.patch_num = 4
            else:
                self.patch_num = 2
            self.patch_len = context_window//(2**self.wt_levels)
            self.stride = stride
            # TODO
            # self.wtconv = WTConv1d(c_in, c_in, wavelet_name=self.wt_name, wt_levels=self.wt_levels)
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        if self.wtconv_patch:
            ...
            if self.deform_patch:
                ...
                patch = z
                # TODO: check this, add these two lines
                x_lep = patch
                PaEN_Loss = 0
