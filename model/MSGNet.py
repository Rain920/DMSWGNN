...
def wavelet_for_Period(x, k=2, level=4, wavelet='db4'):
    ...
    # Calculate the energy of each detail coefficient
    energy = np.abs(cc).mean(0).mean(-1)
    
    # Find the top k periods
    top_list = np.argsort(energy)[-k:]
    period = x.shape[1] // (top_list)

    batch_energy = np.abs(cc).mean(-1)[:, top_list]
    
    # Convert back to torch tensor
    period = torch.tensor(period).to(x.device)
    batch_energy = torch.tensor(batch_energy).to(x.device)
    
    return period, batch_energy
...
def FFT_for_Period(x, k=2):
    ...
    frequency_list[0] = 0 # Remove the DC component(because it is the length of T)
...
class ScaleGraphBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # scale_list, scale_weight = wavelet_for_Period(x, self.k)
